...
public enum PopoverOption {
  ...
  case color(UIColor)
  case upPadding(CGFloat)//TruckerPath
}
...
open class Popover: UIView {
  ...
  open var upPadding: CGFloat = 0.0
  ...
  open func setOptions(_ options: [PopoverOption]?){
    if let options = options {
      for option in options {
		...
		case let .upPadding(value):
			self.upPadding = value
        }
      }
    }
  }
...
//        anchorPoint = CGPoint(x: upPadding / (upPadding + self.arrowSize.height + frame.size.width), y: 0.5)
  ...
  open func show(_ contentView: UIView, fromView: UIView) {
	...
	open func show(_ contentView: UIView, fromView: UIView, inView: UIView, blockOverlay: Bool = true) {
		let point: CGPoint
		switch self.popoverType {
		case .up:
        ...
        case .left:
            point = inView.convert(CGPoint(x: fromView.frame.origin.x, y: fromView.frame.origin.y + 0.5 * fromView.frame.height), from: fromView.superview)
        case .right:
            point = inView.convert(CGPoint(x: fromView.frame.origin.x + fromView.frame.size.width, y: fromView.frame.origin.y + 0.5 * fromView.frame.height), from: fromView.superview)
        }
		self.show(contentView, point: point, inView: inView, blockOverlay: blockOverlay)
	}
...
open func show(_ contentView: UIView, point: CGPoint, inView: UIView, blockOverlay: Bool = true) {
